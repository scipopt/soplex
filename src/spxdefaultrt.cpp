...
/*                                                                           */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
#pragma ident "@(#) $Id: spxdefaultrt.cpp,v 1.13 2002/07/26 08:14:29 bzfkocht Exp $"
...
SPxId SPxDefaultRT::selectEnter(Real& max)
{
   ...
   for( j = 0; j < pidx.size(); ++j )
   {
      ...
      if (isNotZero(x, epsilon))
      {
         ...
         if (fabs(bound[i]) < infinity)
         {
            y = (bound[i] - pvec[i] + d) / x;
   ...
   if (enterId.isValid() && solver()->isBasic(enterId))
   {
      DEBUG({ std::cout << "isValid() && isBasic(): max=" << max
                        << std::endl; });
      if (cnum >= 0)
         solver()->coPvec().delta().clearNum(cnum);
      else
