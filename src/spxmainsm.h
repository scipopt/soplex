...
/*                                                                           */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
#pragma ident "@(#) $Id: spxmainsm.h,v 1.1 2006/02/02 18:34:29 bzftuchs Exp $"
...
public:
   ...
   /// get optimal basis.
   virtual void getBasis(const SPxSolver::VarStatus rows[], const SPxSolver::VarStatus cols[]) const
   {
      assert(m_postsolved);
      rows = m_rBasisStat.get_const_ptr();
      cols = m_cBasisStat.get_const_ptr();
   }   
   //@}
};
} // namespace soplex
