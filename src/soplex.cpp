...
template <>
bool SoPlexBase<Real>::areLPsInSync(const bool checkVecVals, const bool checkMatVals,
                                    const bool quiet) const;

template <>
typename SoPlexBase<Real>::Settings& SoPlexBase<Real>::Settings::operator=
...
template <>
typename SoPlexBase<Real>::RangeType SoPlexBase<Real>::_rangeTypeReal(const Real& lower,
...
template <>
typename SoPlexBase<Real>::RangeType SoPlexBase<Real>::_rangeTypeRational(const Rational& lower,
...
template <>
bool SoPlexBase<Real>::_readFileReal(const char* filename, NameSet* rowNames, NameSet* colNames,
                                     DIdxSet* intVars);
...
SoPlexBase<Real>::Settings::BoolParam::BoolParam()
{
   ...
   description[SoPlexBase<Real>::USEDECOMPDUALSIMPLEX] =
   ...
   description[SoPlexBase<Real>::USECOMPDUAL] =
      "should the dual of the complementary problem be used in the decomposition simplex?";
   ...
   description[SoPlexBase<Real>::EXPLICITVIOL] =
   ...
   description[SoPlexBase<Real>::FULLPERTURBATION] =
...
SoPlexBase<R>::Settings::IntParam::IntParam()
{
   ...
   description[SoPlexBase<Real>::PRINTBASISMETRIC] =
...
template <>
void SoPlexBase<Real>::addRowRational(const mpq_t* lhs, const mpq_t* rowValues,
                                      const int* rowIndices, const int rowSize, const mpq_t* rhs)
{
   assert(_rationalLP != 0);
...
/// adds a set of rows
template <>
void SoPlexBase<Real>::addRowsRational(const mpq_t* lhs, const mpq_t* rowValues,
                                       const int* rowIndices, const int* rowStarts, const int* rowLengths, const int numRows,
                                       const int numValues, const mpq_t* rhs)
{
   assert(_rationalLP != 0);
...
template <>
void SoPlexBase<Real>::addColRational(const mpq_t* obj, const mpq_t* lower, const mpq_t* colValues,
                                      const int* colIndices, const int colSize, const mpq_t* upper)
{
   ...
   if(intParam(SoPlexBase<Real>::SYNCMODE) == SYNCMODE_AUTO)
      _addColReal(Real(maxObjRational(i)) * (intParam(SoPlexBase<Real>::OBJSENSE) ==
                                             SoPlexBase<Real>::OBJSENSE_MAXIMIZE ? 1.0 : -1.0),
...
template <>
void SoPlexBase<Real>::addColsRational(const mpq_t* obj, const mpq_t* lower, const mpq_t* colValues,
                                       ...
                                       const int numValues, const mpq_t* upper)
{
   ...
   if(intParam(SoPlexBase<Real>::SYNCMODE) == SYNCMODE_AUTO)
   {
      ...
      for(int i = numColsRational() - numCols; i < numColsRational(); i++)
         lpcolset.add(Real(maxObjRational(i)) * (intParam(SoPlexBase<Real>::OBJSENSE) ==
                                                 SoPlexBase<Real>::OBJSENSE_MAXIMIZE ? 1.0 : -1.0),
                      Real(lowerRational(i)), DSVectorReal(_rationalLP->colVector(i)), Real(upperRational(i)));
...
/// changes vectors of column bounds to \p lower and \p upper
template <>
void SoPlexBase<Real>::changeBoundsRational(const VectorRational& lower,
...
typename SPxSolverBase<Real>::Status SoPlexBase<Real>::optimize()
{
   ...
   else if(intParam(SoPlexBase<Real>::SOLVEMODE) == SOLVEMODE_REAL
           || (intParam(SoPlexBase<Real>::SOLVEMODE) == SOLVEMODE_AUTO
               && GE(realParam(SoPlexBase<Real>::FEASTOL), 1e-9) && GE(realParam(SoPlexBase<Real>::OPTTOL), 1e-9)))
   {
      ...
      if(realParam(SoPlexBase<Real>::FEASTOL) <
            _currentSettings->realParam.lower[SoPlexBase<Real>::FPFEASTOL])
      {
      ...
      if(realParam(SoPlexBase<Real>::OPTTOL) <
            _currentSettings->realParam.lower[SoPlexBase<Real>::FPOPTTOL])
      {
...
bool SoPlexBase<Real>::getRedCostViolation(Real& maxviol, Real& sumviol)
{
   ...
   for(int c = numCols() - 1; c >= 0; c--)
   {
      ...
      {
         if(colStatus != SPxSolverBase<Real>::ON_UPPER && colStatus != SPxSolverBase<Real>::FIXED
               && redcost[c] < 0.0)
         {
            sumviol += -redcost[c];
         ...
         if(colStatus != SPxSolverBase<Real>::ON_LOWER && colStatus != SPxSolverBase<Real>::FIXED
               && redcost[c] > 0.0)
         {
            sumviol += redcost[c];
...
bool SoPlexBase<Real>::getDualViolation(Real& maxviol, Real& sumviol)
{
   ...
   for(int r = numRows() - 1; r >= 0; r--)
   {
      ...
      if(intParam(SoPlexBase<Real>::OBJSENSE) == OBJSENSE_MINIMIZE)
      {
         if(rowStatus != SPxSolverBase<Real>::ON_UPPER && rowStatus != SPxSolverBase<Real>::FIXED
               && dual[r] < 0.0)
         {
            sumviol += -dual[r];
         ...
         if(rowStatus != SPxSolverBase<Real>::ON_LOWER && rowStatus != SPxSolverBase<Real>::FIXED
               && dual[r] > 0.0)
         {
            sumviol += dual[r];
...
bool SoPlexBase<Real>::getRedCostViolationRational(Rational& maxviol, Rational& sumviol)
{
   ...
   for(int c = numCols() - 1; c >= 0; c--)
   {
      ...
      assert(!_hasBasis || basisColStatus(c) != SPxSolverBase<Real>::ON_LOWER
             || _solRational._primal[c] == lowerRational(c));
      assert(!_hasBasis || basisColStatus(c) != SPxSolverBase<Real>::ON_UPPER
             || _solRational._primal[c] == upperRational(c));
      assert(!_hasBasis || basisColStatus(c) != SPxSolverBase<Real>::FIXED
             || _solRational._primal[c] == lowerRational(c));
      assert(!_hasBasis || basisColStatus(c) != SPxSolverBase<Real>::FIXED
             || _solRational._primal[c] == upperRational(c));
...
bool SoPlexBase<Real>::getDualViolationRational(Rational& maxviol, Rational& sumviol)
{
   ...
   for(int r = numRows() - 1; r >= 0; r--)
   {
      ...
      assert(!_hasBasis || basisRowStatus(r) != SPxSolverBase<Real>::ON_LOWER
             || _solRational._slacks[r] <= lhsRational(r) + _rationalFeastol);
      assert(!_hasBasis || basisRowStatus(r) != SPxSolverBase<Real>::ON_UPPER
             || _solRational._slacks[r] >= rhsRational(r) - _rationalFeastol);
      assert(!_hasBasis || basisRowStatus(r) != SPxSolverBase<Real>::FIXED
             || _solRational._slacks[r] <= lhsRational(r) + _rationalFeastol);
      assert(!_hasBasis || basisRowStatus(r) != SPxSolverBase<Real>::FIXED
             || _solRational._slacks[r] >= rhsRational(r) - _rationalFeastol);
...
/// gets current basis
template <>
void SoPlexBase<Real>::getBasis(typename SPxSolverBase<Real>::VarStatus rows[],
                                typename SPxSolverBase<Real>::VarStatus cols[]) const
{
   // if no basis is available, return slack basis
   if(!hasBasis())
   {
...
template <>
bool SoPlexBase<Real>::getBasisInverseRowReal(int r, Real* coef, int* inds, int* ninds,
      bool unscale)
{
   assert(r >= 0);
   assert(r < numRows());
...
template <>
bool SoPlexBase<Real>::getBasisInverseColReal(int c, Real* coef, int* inds, int* ninds,
      bool unscale)
{
   assert(c >= 0);
   assert(c < numRows());
...
{
   VectorReal v(numRows(), rhs);
   VectorReal x(numRows(), sol);
...
/// on success
template <>
bool SoPlexBase<Real>::getBasisInverseTimesVecRational(const SVectorRational& rhs,
...
template <>
void SoPlexBase<Real>::setBasis(const typename SPxSolverBase<Real>::VarStatus rows[],
                                const typename SPxSolverBase<Real>::VarStatus cols[])
{
   _rationalLUSolver.clear();
...
template <>
bool SoPlexBase<Real>::readFile(const char* filename, NameSet* rowNames, NameSet* colNames,
                                DIdxSet* intVars)
{
   bool success = false;
...
template <>
bool SoPlexBase<Real>::writeFile(const char* filename, const NameSet* rowNames,
...
template <>
bool SoPlexBase<Real>::writeFileReal(const char* filename, const NameSet* rowNames,
                                     const NameSet* colNames, const DIdxSet* intVars, const bool unscale) const
{
...
template <>
bool SoPlexBase<Real>::writeFileRational(const char* filename, const NameSet* rowNames,
...
template <>
bool SoPlexBase<Real>::writeDualFileReal(const char* filename, const NameSet* rowNames,
      const NameSet* colNames, const DIdxSet* intVars) const
{
   SPxLPReal dualLP;
...
template <>
bool SoPlexBase<Real>::readBasisFile(const char* filename, const NameSet* rowNames,
   ...
   for(int i = 0; i < numCols; i++)
   {
      if(lowerRealInternal(i) == upperRealInternal(i))
         _basisStatusCols[i] = SPxSolverBase<Real>::FIXED;
              ...
              && upperRealInternal(i) >= double(realParam(SoPlexBase<Real>::INFTY)))
...
template <>
bool SoPlexBase<Real>::writeBasisFile(const char* filename, const NameSet* rowNames,
   ...
   else
   {
      ...
      for(int col = 0; col < numCols; col++)
      {
         ...
         if(_basisStatusCols[col] == SPxSolverBase<Real>::BASIC)
         {
            ...
            if(_basisStatusRows[row] == SPxSolverBase<Real>::ON_UPPER && (!cpxFormat
                  || _rowTypes[row] == SoPlexBase<Real>::RANGETYPE_BOXED))
               file << " XU ";
            else
               file << " XL ";
...
template <>
void SoPlexBase<Real>::writeStateReal(const char* filename, const NameSet* rowNames,
                                      const NameSet* colNames, const bool cpxFormat) const
{
   std::string ofname;
...
/// default names are used
template <>
void SoPlexBase<Real>::writeStateRational(const char* filename, const NameSet* rowNames,
      const NameSet* colNames, const bool cpxFormat) const
{
   std::string ofname;
...
bool SoPlexBase<Real>::setIntParam(const IntParam param, const int value, const bool init)
{
   ...
   case SoPlexBase<Real>::OBJSENSE:
      ...
      _realLP->changeSense(value == SoPlexBase<Real>::OBJSENSE_MAXIMIZE ? SPxLPReal::MAXIMIZE :
                           SPxLPReal::MINIMIZE);

      if(_rationalLP != 0)
         _rationalLP->changeSense(value == SoPlexBase<Real>::OBJSENSE_MAXIMIZE ? SPxLPRational::MAXIMIZE :
   ...
   case SoPlexBase<Real>::REPRESENTATION:
      if(value != SoPlexBase<Real>::REPRESENTATION_COLUMN
            && value != SoPlexBase<Real>::REPRESENTATION_ROW && value != SoPlexBase<Real>::REPRESENTATION_AUTO)
         return false;
   ...
   case SoPlexBase<Real>::FACTOR_UPDATE_TYPE:
      if(value != SoPlexBase<Real>::FACTOR_UPDATE_TYPE_ETA
            && value != SoPlexBase<Real>::FACTOR_UPDATE_TYPE_FT)
         return false;

      _slufactor.setUtype(value == SoPlexBase<Real>::FACTOR_UPDATE_TYPE_ETA ? SLUFactor::ETA :
                          SLUFactor::FOREST_TOMLIN);
      break;
...
template <>
bool SoPlexBase<Real>::setRationalParam(const RationalParam param, const Rational value,
                                        const bool init)
{
   assert(param >= 0);
   assert(param < RATIONALPARAM_COUNT);
          ...
          if( param >= INTPARAM_COUNT )
          ...
          else if(str.substr(4) == _currentSettings->intParam.name[param])
            {
              ...
              if(setIntParam((SoPlexBase<Real>::IntParam)param, boost::any_cast<int>(val), false))
                ...
                {
                  MSG_INFO1( spxout, spxout << "Error parsing setting string: invalid value <" << boost::any_cast<int>(val) << "> for int parameter <" <<str << ">.\n" );
                  return false;
                }
            }
        }
      ...
      for( int param = 0; ; param++ )
        {
          if( param >= REALPARAM_COUNT )
            {
              MSG_INFO1( spxout, spxout << "Error parsing setting string: unknown parameter name <" << boost::any_cast<Real>(val) << ">.\n" );
              return false;
            }
          else if(str.substr(5) == SoPlexBase<Real>::_currentSettings->realParam.name[param])
            {
              ...
              if( setRealParam((SoPlexBase<Real>::RealParam)param, value) )
                ...
                {
                  MSG_INFO1( spxout, spxout << "Error parsing setting string: invalid value <" << value << "> for R parameter <" << str << ">.\n" );
                  return false;
                }
            }
        }
...
void SoPlexBase<Real>::printStatistics(std::ostream& os)
{
   ...
   os << "Objective sense     : " << (intParam(SoPlexBase<Real>::OBJSENSE) ==
                                      SoPlexBase<Real>::OBJSENSE_MINIMIZE ? "minimize\n" : "maximize\n");
                                    ...
                                    const bool quiet) const
{
   ...
   {
      MSG_INFO1(spxout, spxout <<
                "The number of Rows in the Real LP does not match the one in the Rational LP."
   ...
   {
      MSG_INFO1(spxout, spxout <<
                "The number of Columns in the Real LP does not match the one in the Rational LP."
   ...
   {
      MSG_INFO1(spxout, spxout <<
                "The number of nonZeros in the Real LP does not match the one in the Rational LP."
   ...
   {
      MSG_INFO1(spxout, spxout <<
                "The dimension of the right hand side vector of the Real LP does not match the one of the Rational LP."
                << " Real LP: " << _realLP->rhs().dim() << "  Rational LP: " << _rationalLP->rhs().dim() <<
                std::endl);
      result = false;
      rhsDimMatch = false;
   ...
   {
      MSG_INFO1(spxout, spxout <<
                "The dimension of the left hand side vector of the Real LP does not match the one of the Rational LP."
                << " Real LP: " << _realLP->lhs().dim() << "  Rational LP: " << _rationalLP->lhs().dim() <<
                std::endl);
      result = false;
      lhsDimMatch = false;
   }
   ...
   {
      MSG_INFO1(spxout, spxout <<
                "The dimension of the objective function vector of the Real LP does not match the one of the Rational LP."
                << " Real LP: " << _realLP->maxObj().dim() << "  Rational LP: " << _rationalLP->maxObj().dim() <<
                std::endl);
      result = false;
   ...
   {
      MSG_INFO1(spxout, spxout <<
                "The objective function sense of the Real LP does not match the one of the Rational LP."
   ...
   {
      MSG_INFO1(spxout, spxout <<
                "The dimension of the upper bound vector of the Real LP does not match the one of the Rational LP."
                << " Real LP: " << _realLP->upper().dim() << "  Rational LP: " << _rationalLP->upper().dim() <<
                std::endl);
      result = false;
      upperDimMatch = false;
   }
   ...
   {
      MSG_INFO1(spxout, spxout <<
                "The dimension of the lower bound vector of the Real LP does not match the one of the Rational LP."
                << " Real LP: " << _realLP->lower().dim() << "  Rational LP: " << _rationalLP->lower().dim() <<
                std::endl);
      result = false;
      lowerDimMatch = false;
   }
   ...
   if(checkVecVals)
   {
      ...
      if(rhsDimMatch)
      {
         ...
         {
            if((GE(_realLP->rhs()[i], realParam(SoPlexBase<Real>::INFTY)) != (_rationalLP->rhs()[i] >=
                  _rationalPosInfty))
                  || (LT(_realLP->rhs()[i], realParam(SoPlexBase<Real>::INFTY))
      ...
      if(lhsDimMatch)
      {
         ...
         {
            if((LE(_realLP->lhs()[i], -realParam(SoPlexBase<Real>::INFTY)) != (_rationalLP->lhs()[i] <=
                  _rationalNegInfty))
                  || (GT(_realLP->lhs()[i], -realParam(SoPlexBase<Real>::INFTY))
            ...
            if(!_rationalLP->maxObj()[i].isAdjacentTo((double)_realLP->maxObj()[i]))
            {
               if(!quiet)
               {
                  MSG_INFO1(spxout, spxout << "Entries number " << i <<
                            ...
                            << " Real LP: " << _realLP->maxObj()[i] << "  Rational LP: " << _rationalLP->maxObj()[i] <<
                            std::endl);
               }
      ...
      if(upperDimMatch)
      {
         ...
         {
            if((GE(_realLP->upper()[i], realParam(SoPlexBase<Real>::INFTY)) != (_rationalLP->upper()[i] >=
                  _rationalPosInfty))
                  || (LT(_realLP->upper()[i], realParam(SoPlexBase<Real>::INFTY))
                      ...
                      && !_rationalLP->upper()[i].isAdjacentTo((double)_realLP->upper()[i])))
            {
               if(!quiet)
               {
                  MSG_INFO1(spxout, spxout << "Entries number " << i << " of the upper bound vectors don't match."
                            << " Real LP: " << _realLP->upper()[i] << "  Rational LP: " << _rationalLP->upper()[i] <<
                            std::endl);
               }
      ...
      if(lowerDimMatch)
      {
         ...
         {
            if((LE(_realLP->lower()[i], -realParam(SoPlexBase<Real>::INFTY)) != (_rationalLP->lower()[i] <=
                  _rationalNegInfty))
                  || (GT(_realLP->lower()[i], -realParam(SoPlexBase<Real>::INFTY))
                      ...
                      && !_rationalLP->lower()[i].isAdjacentTo((double)_realLP->lower()[i])))
            {
               if(!quiet)
               {
                  MSG_INFO1(spxout, spxout << "Entries number " << i << " of the lower bound vectors don't match."
                            << " Real LP: " << _realLP->lower()[i] << "  Rational LP: " << _rationalLP->lower()[i] <<
                            std::endl);
               }
            ...
            if(!_rationalLP->colVector(i)[j].isAdjacentTo((double)_realLP->colVector(i)[j]))
            {
               if(!quiet)
               {
                  MSG_INFO1(spxout, spxout << "Entries number " << j << " of column number " << i << " don't match."
                            << " Real LP: " << _realLP->colVector(i)[j] << "  Rational LP: " << _rationalLP->colVector(
                               i)[j] << std::endl);
               }
...
template <>
typename SoPlexBase<Real>::RangeType SoPlexBase<Real>::_switchRangeType(
   const SoPlexBase<Real>::RangeType& rangeType) const
{
...
void SoPlexBase<Real>::_changeRowReal(int i, const LPRowReal& lprow)
{
   ...
   else if(_hasBasis)
   {
      if(_basisStatusRows[i] != SPxSolverBase<Real>::BASIC)
         _hasBasis = false;
      else if(_basisStatusRows[i] == SPxSolverBase<Real>::ON_LOWER
              && lprow.lhs() <= -realParam(SoPlexBase<Real>::INFTY))
         _basisStatusRows[i] = (lprow.rhs() < realParam(SoPlexBase<Real>::INFTY)) ?
                               SPxSolverBase<Real>::ON_UPPER : SPxSolverBase<Real>::ZERO;
      else if(_basisStatusRows[i] == SPxSolverBase<Real>::ON_UPPER
              && lprow.rhs() >= realParam(SoPlexBase<Real>::INFTY))
         _basisStatusRows[i] = (lprow.lhs() > -realParam(SoPlexBase<Real>::INFTY)) ?
                               SPxSolverBase<Real>::ON_LOWER : SPxSolverBase<Real>::ZERO;
   }
...
void SoPlexBase<Real>::_changeLhsReal(const VectorReal& lhs)
{
   ...
   else if(_hasBasis)
   {
      for(int i = numRows() - 1; i >= 0; i--)
      {
         if(_basisStatusRows[i] == SPxSolverBase<Real>::ON_LOWER
               && lhs[i] <= -realParam(SoPlexBase<Real>::INFTY))
            _basisStatusRows[i] = (rhsReal(i) < realParam(SoPlexBase<Real>::INFTY)) ?
                                  SPxSolverBase<Real>::ON_UPPER : SPxSolverBase<Real>::ZERO;
      }
   }
}
...
void SoPlexBase<Real>::_changeLhsReal(int i, const Real& lhs)
{
   ...
   else if(_hasBasis && _basisStatusRows[i] == SPxSolverBase<Real>::ON_LOWER
           && lhs <= -realParam(SoPlexBase<Real>::INFTY))
      _basisStatusRows[i] = (rhsReal(i) < realParam(SoPlexBase<Real>::INFTY)) ?
                            SPxSolverBase<Real>::ON_UPPER : SPxSolverBase<Real>::ZERO;
...
void SoPlexBase<Real>::_changeRhsReal(const VectorReal& rhs)
{
   ...
   else if(_hasBasis)
   {
      for(int i = numRows() - 1; i >= 0; i--)
      {
         if(_basisStatusRows[i] == SPxSolverBase<Real>::ON_UPPER
               && rhs[i] >= realParam(SoPlexBase<Real>::INFTY))
            _basisStatusRows[i] = (lhsReal(i) > -realParam(SoPlexBase<Real>::INFTY)) ?
                                  SPxSolverBase<Real>::ON_LOWER : SPxSolverBase<Real>::ZERO;
      }
   }
}
...
void SoPlexBase<Real>::_changeRhsReal(int i, const Real& rhs)
{
   ...
   else if(_hasBasis && _basisStatusRows[i] == SPxSolverBase<Real>::ON_UPPER
           && rhs >= realParam(SoPlexBase<Real>::INFTY))
      _basisStatusRows[i] = (lhsReal(i) > -realParam(SoPlexBase<Real>::INFTY)) ?
                            SPxSolverBase<Real>::ON_LOWER : SPxSolverBase<Real>::ZERO;
}
...
void SoPlexBase<Real>::_changeRangeReal(const VectorReal& lhs, const VectorReal& rhs)
{
   ...
   else if(_hasBasis)
   {
      for(int i = numRows() - 1; i >= 0; i--)
      {
         if(_basisStatusRows[i] == SPxSolverBase<Real>::ON_LOWER
               && lhs[i] <= -realParam(SoPlexBase<Real>::INFTY))
            _basisStatusRows[i] = (rhs[i] < realParam(SoPlexBase<Real>::INFTY)) ?
                                  SPxSolverBase<Real>::ON_UPPER : SPxSolverBase<Real>::ZERO;
         else if(_basisStatusRows[i] == SPxSolverBase<Real>::ON_UPPER
                 && rhs[i] >= realParam(SoPlexBase<Real>::INFTY))
            _basisStatusRows[i] = (lhs[i] > -realParam(SoPlexBase<Real>::INFTY)) ?
                                  SPxSolverBase<Real>::ON_LOWER : SPxSolverBase<Real>::ZERO;
      }
   }
}
...
void SoPlexBase<Real>::_changeRangeReal(int i, const Real& lhs, const Real& rhs)
{
   ...
   else if(_hasBasis)
   {
      if(_basisStatusRows[i] == SPxSolverBase<Real>::ON_LOWER
            && lhs <= -realParam(SoPlexBase<Real>::INFTY))
         _basisStatusRows[i] = (rhs < realParam(SoPlexBase<Real>::INFTY)) ? SPxSolverBase<Real>::ON_UPPER :
                               SPxSolverBase<Real>::ZERO;
      else if(_basisStatusRows[i] == SPxSolverBase<Real>::ON_UPPER
              && rhs >= realParam(SoPlexBase<Real>::INFTY))
         _basisStatusRows[i] = (lhs > -realParam(SoPlexBase<Real>::INFTY)) ? SPxSolverBase<Real>::ON_LOWER :
                               SPxSolverBase<Real>::ZERO;
   }
}
...
void SoPlexBase<Real>::_changeColReal(int i, const LPColReal& lpcol)
{
   ...
   else if(_hasBasis)
   {
      if(_basisStatusCols[i] == SPxSolverBase<Real>::BASIC)
         _hasBasis = false;
      else if(_basisStatusCols[i] == SPxSolverBase<Real>::ON_LOWER
              && lpcol.lower() <= -realParam(SoPlexBase<Real>::INFTY))
         _basisStatusCols[i] = (lpcol.upper() < realParam(SoPlexBase<Real>::INFTY)) ?
                               SPxSolverBase<Real>::ON_UPPER : SPxSolverBase<Real>::ZERO;
      else if(_basisStatusCols[i] == SPxSolverBase<Real>::ON_UPPER
              && lpcol.upper() >= realParam(SoPlexBase<Real>::INFTY))
         _basisStatusCols[i] = (lpcol.lower() > -realParam(SoPlexBase<Real>::INFTY)) ?
                               SPxSolverBase<Real>::ON_LOWER : SPxSolverBase<Real>::ZERO;
   }
...
void SoPlexBase<Real>::_changeLowerReal(const VectorReal& lower)
{
   ...
   else if(_hasBasis)
   {
      for(int i = numCols() - 1; i >= 0; i--)
      {
         if(_basisStatusCols[i] == SPxSolverBase<Real>::ON_LOWER
               && lower[i] <= -realParam(SoPlexBase<Real>::INFTY))
            _basisStatusCols[i] = (upperReal(i) < realParam(SoPlexBase<Real>::INFTY)) ?
                                  SPxSolverBase<Real>::ON_UPPER : SPxSolverBase<Real>::ZERO;
      }
   }
}
...
void SoPlexBase<Real>::_changeLowerReal(int i, const Real& lower)
{
   ...
   else if(_hasBasis && _basisStatusCols[i] == SPxSolverBase<Real>::ON_LOWER
           && lower <= -realParam(SoPlexBase<Real>::INFTY))
      _basisStatusCols[i] = (upperReal(i) < realParam(SoPlexBase<Real>::INFTY)) ?
                            SPxSolverBase<Real>::ON_UPPER : SPxSolverBase<Real>::ZERO;
}
...
void SoPlexBase<Real>::_changeUpperReal(const VectorReal& upper)
{
   ...
   else if(_hasBasis)
   {
      for(int i = numCols() - 1; i >= 0; i--)
      {
         if(_basisStatusCols[i] == SPxSolverBase<Real>::ON_UPPER
               && upper[i] >= realParam(SoPlexBase<Real>::INFTY))
            _basisStatusCols[i] = (lowerReal(i) > -realParam(SoPlexBase<Real>::INFTY)) ?
                                  SPxSolverBase<Real>::ON_LOWER : SPxSolverBase<Real>::ZERO;
      }
   }
}
...
void SoPlexBase<Real>::_changeUpperReal(int i, const Real& upper)
{
   ...
   else if(_hasBasis &&  _basisStatusCols[i] == SPxSolverBase<Real>::ON_UPPER
           && upper >= realParam(SoPlexBase<Real>::INFTY))
      _basisStatusCols[i] = (lowerReal(i) > -realParam(SoPlexBase<Real>::INFTY)) ?
                            SPxSolverBase<Real>::ON_LOWER : SPxSolverBase<Real>::ZERO;
}
...
void SoPlexBase<Real>::_changeBoundsReal(const VectorReal& lower, const VectorReal& upper)
{
   ...
   else if(_hasBasis)
   {
      for(int i = numCols() - 1; i >= 0; i--)
      {
         if(_basisStatusCols[i] == SPxSolverBase<Real>::ON_LOWER
               && lower[i] <= -realParam(SoPlexBase<Real>::INFTY))
            _basisStatusCols[i] = (upper[i] < realParam(SoPlexBase<Real>::INFTY)) ?
                                  SPxSolverBase<Real>::ON_UPPER : SPxSolverBase<Real>::ZERO;
         else if(_basisStatusCols[i] == SPxSolverBase<Real>::ON_UPPER
                 && upper[i] >= realParam(SoPlexBase<Real>::INFTY))
            _basisStatusCols[i] = (lower[i] > -realParam(SoPlexBase<Real>::INFTY)) ?
                                  SPxSolverBase<Real>::ON_LOWER : SPxSolverBase<Real>::ZERO;
      }
   }
}
...
void SoPlexBase<Real>::_changeBoundsReal(int i, const Real& lower, const Real& upper)
{
   ...
   else if(_hasBasis)
   {
      if(_basisStatusCols[i] == SPxSolverBase<Real>::ON_LOWER
            && lower <= -realParam(SoPlexBase<Real>::INFTY))
         _basisStatusCols[i] = (upper < realParam(SoPlexBase<Real>::INFTY)) ? SPxSolverBase<Real>::ON_UPPER :
                               SPxSolverBase<Real>::ZERO;
      else if(_basisStatusCols[i] == SPxSolverBase<Real>::ON_UPPER
              && upper >= realParam(SoPlexBase<Real>::INFTY))
         _basisStatusCols[i] = (lower > -realParam(SoPlexBase<Real>::INFTY)) ?
                               SPxSolverBase<Real>::ON_LOWER : SPxSolverBase<Real>::ZERO;
   }
}
...
void SoPlexBase<Real>::_solveRealLPAndRecordStatistics()
{
   ...
   if(realParam(SoPlexBase<Real>::TIMELIMIT) < realParam(SoPlexBase<Real>::INFTY))
      _solver.setTerminationTime(realParam(SoPlexBase<Real>::TIMELIMIT) -
   ...
   if((intParam(SoPlexBase<Real>::REPRESENTATION) == SoPlexBase<Real>::REPRESENTATION_COLUMN
         || (intParam(SoPlexBase<Real>::REPRESENTATION) == SoPlexBase<Real>::REPRESENTATION_AUTO
             && (_solver.nCols() + 1) * realParam(SoPlexBase<Real>::REPRESENTATION_SWITCH) >=
             (_solver.nRows() + 1)))
   ...
   // set correct type
   if(((intParam(ALGORITHM) == SoPlexBase<Real>::ALGORITHM_PRIMAL
         && _solver.rep() == SPxSolverBase<Real>::COLUMN)
         || (intParam(ALGORITHM) == SoPlexBase<Real>::ALGORITHM_DUAL
             && _solver.rep() == SPxSolverBase<Real>::ROW))
         && _solver.type() != SPxSolverBase<Real>::ENTER)
   {
      _solver.setType(SPxSolverBase<Real>::ENTER);
   }
   else if(((intParam(ALGORITHM) == SoPlexBase<Real>::ALGORITHM_DUAL
             && _solver.rep() == SPxSolverBase<Real>::COLUMN)
            || (intParam(ALGORITHM) == SoPlexBase<Real>::ALGORITHM_PRIMAL
                && _solver.rep() == SPxSolverBase<Real>::ROW))
                                     ...
                                     DIdxSet* intVars)
{
   ...
   if(success)
   {
      setIntParam(SoPlexBase<Real>::OBJSENSE,
                  (_realLP->spxSense() == SPxLPReal::MAXIMIZE ? SoPlexBase<Real>::OBJSENSE_MAXIMIZE :
                   SoPlexBase<Real>::OBJSENSE_MINIMIZE), true);
      ...
      DIdxSet* intVars)
{
   ...
   {
      setIntParam(SoPlexBase<Real>::OBJSENSE,
                  (_rationalLP->spxSense() == SPxLPRational::MAXIMIZE ? SoPlexBase<Real>::OBJSENSE_MAXIMIZE :
                   SoPlexBase<Real>::OBJSENSE_MINIMIZE), true);
   ...
   if(strncmp(paramTypeString, "real", 4) == 0)
   {
      for(int param = 0; ; param++)
      {
         ...
         else if(strncmp(paramName, _currentSettings->realParam.name[param].c_str(), SET_MAX_LINE_LEN) == 0)
         {
            ...
            {
               MSG_INFO1(spxout, spxout << "Error parsing settings file: invalid value <" << paramValueString <<
                         "> for real parameter <" << paramName << "> in line " << lineNumber << ".\n");
               return false;
            }
         }
      }
      ...
      for(int param = 0; ; param++)
      {
                         ...
                         SET_MAX_LINE_LEN) == 0)
         {
            ...
            if(readStringRational(paramValueString, value)
                  && setRationalParam((SoPlexBase<Real>::RationalParam)param, value))
               break;
            else
            {
               MSG_INFO1(spxout, spxout << "Error parsing settings file: invalid value <" << paramValueString <<
