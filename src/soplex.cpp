...
SoPlex::Settings::BoolParam::BoolParam()
{
   ...
   description[SoPlex::USEDECOMPDUALSIMPLEX] =
   ...
   description[SoPlex::USECOMPDUAL] =
      "should the dual of the complementary problem be used in the decomposition simplex?";
   ...
   description[SoPlex::EXPLICITVIOL] =
   ...
   description[SoPlex::ACCEPTCYCLING] =
      "should cycling solutions be accepted during iterative refinement?";
...
SoPlex::Settings::IntParam::IntParam()
{
   ...
   description[SoPlex::REPRESENTATION] =
   ...
   description[SoPlex::FACTOR_UPDATE_TYPE] =
   ...
   description[SoPlex::FACTOR_UPDATE_MAX] =
   ...
   description[SoPlex::RATIOTESTER] =
   ...
   description[SoPlex::HYPER_PRICING] =
   ...
   description[SoPlex::RATFAC_MINSTALLS] =
      "minimum number of stalling refinements since last pivot to trigger rational factorization";
   ...
   description[SoPlex::LEASTSQ_MAXROUNDS] =
      "maximum number of conjugate gradient iterations in least square scaling";
   ...
   description[SoPlex::SOLUTION_POLISHING] =
   ...
   description[SoPlex::DECOMP_ITERLIMIT] =
   ...
   description[SoPlex::DECOMP_MAXADDEDROWS] =
   ...
   description[SoPlex::DECOMP_DISPLAYFREQ] =
   ...
   description[SoPlex::DECOMP_VERBOSITY] =
   ...
   description[SoPlex::PRINTCONDITION] =
...
SoPlex::Settings::RealParam::RealParam()
{
   ...
   description[SoPlex::SPARSITY_THRESHOLD] =
   ...
   description[SoPlex::REPRESENTATION_SWITCH] =
      "threshold on number of rows vs. number of columns for switching from column to row representations in auto mode";
   ...
   description[SoPlex::REFAC_BASIS_NNZ] =
      "refactor threshold for nonzeros in last factorized basis matrix compared to updated basis matrix";
   ...
   description[SoPlex::REFAC_UPDATE_FILL] =
      "refactor threshold for fill-in in current factor update compared to fill-in in last factorization";
   ...
   description[SoPlex::REFAC_MEM_FACTOR] =
      "refactor threshold for memory growth in factorization since last refactorization";
   ...
   description[SoPlex::LEASTSQ_ACRCY] =
      "accuracy of conjugate gradient method in least squares scaling (higher value leads to more iterations)";
...
/// adds a set of rows
void SoPlex::addRowsRational(const mpq_t* lhs, const mpq_t* rowValues, const int* rowIndices,
                            ...
                            const int* colIndices, const int colSize, const mpq_t* upper)
{
   ...
   if(intParam(SoPlex::SYNCMODE) == SYNCMODE_AUTO)
      _addColReal(Real(maxObjRational(i)) * (intParam(SoPlex::OBJSENSE) == SoPlex::OBJSENSE_MAXIMIZE ?
                                             1.0 : -1.0),
...
void SoPlex::addColsRational(const mpq_t* obj, const mpq_t* lower, const mpq_t* colValues,
                             ...
                             const int numValues, const mpq_t* upper)
{
   ...
   if(intParam(SoPlex::SYNCMODE) == SYNCMODE_AUTO)
   {
      ...
      for(int i = numColsRational() - numCols; i < numColsRational(); i++)
         lpcolset.add(Real(maxObjRational(i)) * (intParam(SoPlex::OBJSENSE) == SoPlex::OBJSENSE_MAXIMIZE ?
                                                 1.0 : -1.0),
                      Real(lowerRational(i)), DSVectorReal(_rationalLP->colVector(i)), Real(upperRational(i)));
...
SPxSolver::Status SoPlex::optimize()
{
   ...
   else if(intParam(SoPlex::SOLVEMODE) == SOLVEMODE_REAL
           || (intParam(SoPlex::SOLVEMODE) == SOLVEMODE_AUTO
...
bool SoPlex::getRedCostViolationRational(Rational& maxviol, Rational& sumviol)
{
   ...
   for(int c = numColsReal() - 1; c >= 0; c--)
   {
      ...
      assert(!_hasBasis || basisColStatus(c) != SPxSolver::ON_LOWER
             || _solRational._primal[c] == lowerRational(c));
      assert(!_hasBasis || basisColStatus(c) != SPxSolver::ON_UPPER
             || _solRational._primal[c] == upperRational(c));
      assert(!_hasBasis || basisColStatus(c) != SPxSolver::FIXED
             || _solRational._primal[c] == lowerRational(c));
      assert(!_hasBasis || basisColStatus(c) != SPxSolver::FIXED
             || _solRational._primal[c] == upperRational(c));
...
bool SoPlex::getDualViolationRational(Rational& maxviol, Rational& sumviol)
{
   ...
   for(int r = numRowsReal() - 1; r >= 0; r--)
   {
      ...
      assert(!_hasBasis || basisRowStatus(r) != SPxSolver::ON_LOWER
             || _solRational._slacks[r] <= lhsRational(r) + _rationalFeastol);
      assert(!_hasBasis || basisRowStatus(r) != SPxSolver::ON_UPPER
             || _solRational._slacks[r] >= rhsRational(r) - _rationalFeastol);
      assert(!_hasBasis || basisRowStatus(r) != SPxSolver::FIXED
             || _solRational._slacks[r] <= lhsRational(r) + _rationalFeastol);
      assert(!_hasBasis || basisRowStatus(r) != SPxSolver::FIXED
             || _solRational._slacks[r] >= rhsRational(r) - _rationalFeastol);
      ...
      else
      {
         ...
         for(int i = 0; i < numRowsReal(); i++)
         {
            ...
            if(idx < 0)
            {
               ...
               assert(idx >= 0);
               assert(idx < numRowsReal());
               assert(!_solver.isRowBasic(idx));

               if(unscale && _solver.isScaled())
               {
                  DSVector r_unscaled(numColsReal());
...
bool SoPlex::writeFileReal(const char* filename, const NameSet* rowNames, const NameSet* colNames,
                           const DIdxSet* intVars, const bool unscale) const
{
...
/// colNames are \c NULL, default names are used; if \p intVars is not \c NULL, the variables contained in it are
/// marked as integer; returns true on success
bool SoPlex::writeFileRational(const char* filename, const NameSet* rowNames,
...
bool SoPlex::writeDualFileReal(const char* filename, const NameSet* rowNames,
                               const NameSet* colNames, const DIdxSet* intVars) const
{
   SPxLPReal dualLP;
   ...
   for(int i = 0; i < numCols; i++)
   {
      if(lowerRealInternal(i) == upperRealInternal(i))
         _basisStatusCols[i] = SPxSolver::FIXED;
              ...
              && upperRealInternal(i) >= double(realParam(SoPlex::INFTY)))
...
bool SoPlex::writeBasisFile(const char* filename, const NameSet* rowNames, const NameSet* colNames,
   ...
   else
   {
      ...
      for(int col = 0; col < numCols; col++)
      {
         ...
         if(_basisStatusCols[col] == SPxSolver::BASIC)
         {
            ...
            if(_basisStatusRows[row] == SPxSolver::ON_UPPER && (!cpxFormat
                  || _rowTypes[row] == SoPlex::RANGETYPE_BOXED))
               file << " XU ";
            else
               file << " XL ";
...
void SoPlex::writeStateReal(const char* filename, const NameSet* rowNames, const NameSet* colNames,
                            const bool cpxFormat) const
{
   std::string ofname;
...
/// writes internal LP, basis information, and parameter settings; if \p rowNames and \p colNames are \c NULL,
/// default names are used
void SoPlex::writeStateRational(const char* filename, const NameSet* rowNames,
...
bool SoPlex::setIntParam(const IntParam param, const int value, const bool init)
{
   ...
   case SoPlex::OBJSENSE:
      ...
      _realLP->changeSense(value == SoPlex::OBJSENSE_MAXIMIZE ? SPxLPReal::MAXIMIZE :
                           SPxLPReal::MINIMIZE);

      if(_rationalLP != 0)
         _rationalLP->changeSense(value == SoPlex::OBJSENSE_MAXIMIZE ? SPxLPRational::MAXIMIZE :
   ...
   case SoPlex::REPRESENTATION:
      if(value != SoPlex::REPRESENTATION_COLUMN && value != SoPlex::REPRESENTATION_ROW
            && value != SoPlex::REPRESENTATION_AUTO)
         return false;
   ...
   case SoPlex::FACTOR_UPDATE_TYPE:
      ...
      _slufactor.setUtype(value == SoPlex::FACTOR_UPDATE_TYPE_ETA ? SLUFactor::ETA :
...
bool SoPlex::saveSettingsFile(const char* filename, const bool onlyChanged) const
{
   ...
   file << "# SoPlex version " << SOPLEX_VERSION / 100 << "." << (SOPLEX_VERSION / 10) % 10 << "." <<
        SOPLEX_VERSION % 10;
      ...
      for(int param = 0; ; param++)
      {
                         ...
                         SET_MAX_LINE_LEN) == 0)
         {
            ...
            if(readStringRational(paramValueString, value)
                  && setRationalParam((SoPlex::RationalParam)param, value))
               break;
            else
            {
               MSG_INFO1(spxout, spxout << "Error parsing setting string: invalid value <" << paramValueString <<
...
void SoPlex::printSolutionStatistics(std::ostream& os)
{
   ...
   if(intParam(SoPlex::CHECKMODE) == CHECKMODE_RATIONAL
         || (intParam(SoPlex::CHECKMODE) == CHECKMODE_AUTO
             && intParam(SoPlex::READMODE) == READMODE_RATIONAL))
   {
      Rational maxviol;
      Rational sumviol;
...
void SoPlex::printStatistics(std::ostream& os)
{
   ...
   os << "Objective sense     : " << (intParam(SoPlex::OBJSENSE) == SoPlex::OBJSENSE_MINIMIZE ?
                                      "minimize\n" : "maximize\n");
...
bool SoPlex::areLPsInSync(const bool checkVecVals, const bool checkMatVals, const bool quiet) const
{
   ...
   if(checkVecVals)
   {
      ...
      if(lhsDimMatch)
      {
         ...
         {
            if((LE(_realLP->lhs()[i], -realParam(SoPlex::INFTY)) != (_rationalLP->lhs()[i] <=
                  _rationalNegInfty))
      ...
      if(upperDimMatch)
      {
         ...
         {
            if((GE(_realLP->upper()[i], realParam(SoPlex::INFTY)) != (_rationalLP->upper()[i] >=
                  _rationalPosInfty))
      ...
      if(lowerDimMatch)
      {
         ...
         {
            if((LE(_realLP->lower()[i], -realParam(SoPlex::INFTY)) != (_rationalLP->lower()[i] <=
                  _rationalNegInfty))
                  || (GT(_realLP->lower()[i], -realParam(SoPlex::INFTY))
...
void SoPlex::_changeRowReal(int i, const LPRowReal& lprow)
{
   ...
   else if(_hasBasis)
   {
      if(_basisStatusRows[i] != SPxSolver::BASIC)
         ...
         _basisStatusRows[i] = (lprow.rhs() < realParam(SoPlex::INFTY)) ? SPxSolver::ON_UPPER :
                               SPxSolver::ZERO;
      else if(_basisStatusRows[i] == SPxSolver::ON_UPPER && lprow.rhs() >= realParam(SoPlex::INFTY))
         _basisStatusRows[i] = (lprow.lhs() > -realParam(SoPlex::INFTY)) ? SPxSolver::ON_LOWER :
                               SPxSolver::ZERO;
   }
   ...
   else if(_hasBasis)
   {
      for(int i = numRowsReal() - 1; i >= 0; i--)
      {
         if(_basisStatusRows[i] == SPxSolver::ON_LOWER && lhs[i] <= -realParam(SoPlex::INFTY))
            _basisStatusRows[i] = (rhsReal(i) < realParam(SoPlex::INFTY)) ? SPxSolver::ON_UPPER :
                                  SPxSolver::ZERO;
      }
   }
}
   ...
   else if(_hasBasis)
   {
      for(int i = numRowsReal() - 1; i >= 0; i--)
      {
         if(_basisStatusRows[i] == SPxSolver::ON_UPPER && rhs[i] >= realParam(SoPlex::INFTY))
            _basisStatusRows[i] = (lhsReal(i) > -realParam(SoPlex::INFTY)) ? SPxSolver::ON_LOWER :
                                  SPxSolver::ZERO;
      }
   }
}
...
void SoPlex::_changeColReal(int i, const LPColReal& lpcol)
{
   ...
   else if(_hasBasis)
   {
      if(_basisStatusCols[i] == SPxSolver::BASIC)
         ...
         _basisStatusCols[i] = (lpcol.upper() < realParam(SoPlex::INFTY)) ? SPxSolver::ON_UPPER :
                               SPxSolver::ZERO;
      else if(_basisStatusCols[i] == SPxSolver::ON_UPPER && lpcol.upper() >= realParam(SoPlex::INFTY))
         _basisStatusCols[i] = (lpcol.lower() > -realParam(SoPlex::INFTY)) ? SPxSolver::ON_LOWER :
                               SPxSolver::ZERO;
   }
   ...
   else if(_hasBasis)
   {
      for(int i = numColsReal() - 1; i >= 0; i--)
      {
         if(_basisStatusCols[i] == SPxSolver::ON_LOWER && lower[i] <= -realParam(SoPlex::INFTY))
            _basisStatusCols[i] = (upperReal(i) < realParam(SoPlex::INFTY)) ? SPxSolver::ON_UPPER :
                                  SPxSolver::ZERO;
      }
   }
}
...
void SoPlex::_changeLowerReal(int i, const Real& lower)
{
   ...
   else if(_hasBasis && _basisStatusCols[i] == SPxSolver::ON_LOWER
   ...
   else if(_hasBasis)
   {
      for(int i = numColsReal() - 1; i >= 0; i--)
      {
         if(_basisStatusCols[i] == SPxSolver::ON_UPPER && upper[i] >= realParam(SoPlex::INFTY))
            _basisStatusCols[i] = (lowerReal(i) > -realParam(SoPlex::INFTY)) ? SPxSolver::ON_LOWER :
                                  SPxSolver::ZERO;
      }
   }
}
...
void SoPlex::_changeUpperReal(int i, const Real& upper)
{
   ...
   else if(_hasBasis &&  _basisStatusCols[i] == SPxSolver::ON_UPPER
           && upper >= realParam(SoPlex::INFTY))
      _basisStatusCols[i] = (lowerReal(i) > -realParam(SoPlex::INFTY)) ? SPxSolver::ON_LOWER :
                            SPxSolver::ZERO;
}
   ...
   else if(_hasBasis)
   {
      for(int i = numColsReal() - 1; i >= 0; i--)
      {
         if(_basisStatusCols[i] == SPxSolver::ON_LOWER && lower[i] <= -realParam(SoPlex::INFTY))
            ...
            _basisStatusCols[i] = (lower[i] > -realParam(SoPlex::INFTY)) ? SPxSolver::ON_LOWER :
                                  SPxSolver::ZERO;
      }
   }
}
...
bool SoPlex::_readFileReal(const char* filename, NameSet* rowNames, NameSet* colNames,
                           DIdxSet* intVars)
{
   assert(_realLP != 0);
   ...
   if(success)
   {
      setIntParam(SoPlex::OBJSENSE, (_realLP->spxSense() == SPxLPReal::MAXIMIZE ?
                                     SoPlex::OBJSENSE_MAXIMIZE : SoPlex::OBJSENSE_MINIMIZE), true);
...
/// integer variables if desired
bool SoPlex::_readFileRational(const char* filename, NameSet* rowNames, NameSet* colNames,
                               DIdxSet* intVars)
{
   // clear statistics
   _statistics->clearAllData();
   ...
   if(success)
   {
      setIntParam(SoPlex::OBJSENSE, (_rationalLP->spxSense() == SPxLPRational::MAXIMIZE ?
                                     SoPlex::OBJSENSE_MAXIMIZE : SoPlex::OBJSENSE_MINIMIZE), true);
