      ...
      if (dupRows[k].size() > 1)
      {
         ...
         if (rowIdx != -1)
         {
            ...
            if (maxLhs > lp.lhs(rowIdx) || minRhs < lp.rhs(rowIdx))
            {
               ...
               lp.changeRange(rowIdx, newLhs, newRhs);      
...
#ifdef DUPLICATE_ROWS
      if (ret == OKAY)
         ret = duplicateRows(lp, again);
#endif
      
#ifdef DUPLICATE_COLS
      if (ret == OKAY)
