...
namespace soplex
{
  ...
  typename SPxSolver<R>::Status SPxSolver<R>::solve()
  {
    ...
    if (!isInitialized())
      {
        /*
          if(SPxBasis<R>::status() <= NO_PROBLEM)
            ...
            if (type() == ENTER)
              {
                ...
                do
                  {
                    ...
                    if (!enterId.isValid())
                      {
                        ...
                        if (  shift() <= epsilon()
                              && ( SPxBasis<R>::status() == SPxBasis<R>::REGULAR 
                                   || SPxBasis<R>::status() == SPxBasis<R>::DUAL 
                                   || SPxBasis<R>::status() == SPxBasis<R>::PRIMAL))
                          {
                          ...
                          if( this->lastUpdate() > 0 && enterFacPivotCount < MAXREFACPIVOTS )
                            {
                              ...
                              if( SPxBasis<R>::status() < SPxBasis<R>::REGULAR )
                                {
                                  MSG_INFO1( (*spxout), (*spxout) << "Something wrong with factorization, Basis status: " << SPxBasis<R>::status() << std::endl; )
                                    stop = true;
                                  break;
                                }
                    ...
                    if( this->lastEntered().isValid() )
                      enterCycleCount = 0;
                    else if( basis().status() != SPxBasis<R>::INFEASIBLE && basis().status() != SPxBasis<R>::UNBOUNDED )
                      {
                        enterCycleCount++;
                ...
                MSG_INFO3( (*spxout),
                           ...
                           << std::endl
                           << "ISOLVE56 stop: " << stop
                           << ", basis status: " << SPxBasis<R>::status() << " (" << int(SPxBasis<R>::status()) << ")"
                      ...
                      if (shift() <= epsilon())
                        {
                          ...
                          MSG_INFO3( (*spxout),
                            ...
                            if (priced && maxinfeas + shift() <= entertol())
                              {
                                setBasisStatus(SPxBasis<R>::OPTIMAL);
                                m_status = OPTIMAL;
                                break;
                              }
                        }
    ...
    if (tmp.length() > leavetol())
      {
        MSG_INFO3( (*spxout), (*spxout) << "ISOLVE93 " << this->iteration() << ":\tcoP error = \t"
          ...
          tmp.clear();
        SPxBasis<R>::coSolve(tmp, *theCoPrhs);
        this->multWithBase(tmp);
        tmp -= *theCoPrhs;
  ...
  bool SPxSolver<R>::terminate()
  {
    ...
    if( SPxBasis<R>::status() >= SPxBasis<R>::OPTIMAL  ||
        SPxBasis<R>::status() <= SPxBasis<R>::SINGULAR )
      {
        m_status = UNKNOWN;
        return true;
      }
    ...
    if( getStartingDecompBasis )
      {
        ...
        if( type() == ENTER && SPxBasis<R>::status() == SPxBasis<R>::DUAL &&
            this->iteration() - this->lastDegenCheck() > getDegenCompOffset()/*iteration() % 10 == 0*/ )
          {
            ...
            if( (degeneracyLevel < degeneracyUB && degeneracyLevel > degeneracyLB) && this->iteration() > this->nRows()*0.2 )
              {
                m_status = ABORT_DECOMP;
                return true;
              }

            if( degeneracyLevel < degeneracyLB && this->iteration() > MINIMUM(getDecompIterationLimit(), int(this->nCols()*iterationFrac)) )
              {
        ...
        else if( type() == LEAVE && this->iteration() > MINIMUM(getDecompIterationLimit(), int(this->nCols()*iterationFrac)) )
          {
            setDecompIterationLimit(0);
            setDegenCompOffset(0);
            m_status = ABORT_EXDECOMP;
  ...
  typename SPxSolver<R>::Status SPxSolver<R>::getPrimalray (Vector& p_vector) const
  {
    ...
    assert(SPxBasis<R>::status() == SPxBasis<R>::UNBOUNDED);
    p_vector.clear();
    p_vector = primalRay;
