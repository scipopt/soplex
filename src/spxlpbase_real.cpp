...
template<>
void SPxLPBase<Real>::changeMaxObj(int i, const Real& newVal, bool scale)
{
   if( scale )
   {
      assert(_isScaled);
...
template<>
void SPxLPBase<Real>::changeLower(int i, const Real& newLower, bool scale)
{
   if( scale )
   {
      assert(_isScaled);
...
template<>
void SPxLPBase<Real>::changeUpper(int i, const Real& newUpper, bool scale)
{
   if( scale )
   {
      assert(_isScaled);
...
template<>
void SPxLPBase<Real>::changeLhs(int i, const Real& newLhs, bool scale)
{
   if( scale )
...
template<>
void SPxLPBase<Real>::changeRhs(int i, const Real& newRhs, bool scale)
{
   if( scale )
...
template<>
void SPxLPBase<Real>::changeRow(int n, const LPRowBase<Real>& newRow, bool scale)
{
   if( n < 0 )
      return;

   int j;
   SVectorBase<Real>& row = rowVector_w(n);
   for( j = row.size() - 1; j >= 0; --j )
   {
      SVectorBase<Real>& col = colVector_w(row.index(j));
   ...
   const SVectorBase<Real>& newrow = newRow.rowVector();
   for( j = newrow.size() - 1; j >= 0; --j )
   {
      ...
      LPRowSetBase<Real>::add2(n, 1, &idx, &val);
      LPColSetBase<Real>::add2(idx, 1, &n, &val);
   }
...
template<>
void SPxLPBase<Real>::changeCol(int n, const LPColBase<Real>& newCol, bool scale)
{
   if( n < 0 )
      return;

   int j;
   SVectorBase<Real>& col = colVector_w(n);
   for( j = col.size() - 1; j >= 0; --j )
   {
      SVectorBase<Real>& row = rowVector_w(col.index(j));
   ...
   const SVectorBase<Real>& newcol = newCol.colVector();
   for( j = newcol.size() - 1; j >= 0; --j )
   {
      ...
      LPColSetBase<Real>::add2(n, 1, &idx, &val);
      LPRowSetBase<Real>::add2(idx, 1, &n, &val);
   }
...
template<>
void SPxLPBase<Real>::doRemoveRow(int j)
{
   const SVectorBase<Real>& vec = rowVector(j);

   // remove row vector from column file
   for( int i = vec.size() - 1; i >= 0; --i )
   {
      SVectorBase<Real>& remvec = colVector_w(vec.index(i));
   ...
   if( j != idx )
   {
      const SVectorBase<Real>& l_vec = rowVector(idx);
      for( int i = l_vec.size() - 1; i >= 0; --i )
      {
         SVectorBase<Real>& movevec = colVector_w(l_vec.index(i));
         movevec.index(movevec.number(idx)) = j;
      }
      if( _isScaled )
      {
         LPRowSetBase<Real>::scaleExp[j] = LPRowSetBase<Real>::scaleExp[nRows() - 1];
         LPRowSetBase<Real>::scaleExp.reSize(nRows() - 1);
      }
   }

   LPRowSetBase<Real>::remove(j);
}

/// Internal helper method.
template<>
void SPxLPBase<Real>::doRemoveRows(int perm[])
{
   int ncols = nCols();

   LPRowSetBase<Real>::remove(perm);

   for( int i = 0; i < ncols; ++i )
   {
      SVectorBase<Real>& vec = colVector_w(i);
...
/// Internal helper method.
template<>
void SPxLPBase<Real>::doRemoveCol(int j)
{
   const SVectorBase<Real>& vec = colVector(j);
   int i;

   // remove column vector from row file
   for( i = vec.size() - 1; i >= 0; --i )
   {
      SVectorBase<Real>& remvec = rowVector_w(vec.index(i));
      remvec.remove(remvec.number(j));
   }
   ...
   if( j != idx )
   {
      const SVectorBase<Real>& l_vec = colVector(idx);
      for( i = l_vec.size() - 1; i >= 0; --i )
      {
         SVectorBase<Real>& movevec = rowVector_w(l_vec.index(i));
         movevec.index(movevec.number(idx)) = j;
      }
   }

   LPColSetBase<Real>::remove(j);
}

/// Internal helper method.
template<>
void SPxLPBase<Real>::doRemoveCols(int perm[])
{
   int nrows = nRows();

   LPColSetBase<Real>::remove(perm);

   for( int i = 0; i < nrows; ++i )
   {
      SVectorBase<Real>& vec = rowVector_w(i);
