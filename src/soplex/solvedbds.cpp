...
void SoPlex::_solveDecompositionDualSimplex()
{
   ...
   while(!stop)
   {
      ...
      stop = decompTerminate(realParam(
                                SoPlex::TIMELIMIT));  // checking whether the algorithm should terminate
...
void SoPlex::_formDecompComplementaryProblem()
{
   ...
   if(boolParam(
            SoPlex::USECOMPDUAL))     // if we use the dual formulation of the complementary problem, we must
...
void SoPlex::_decompSimplifyAndSolve(SPxSolver& solver, SLUFactor& sluFactor, bool fromScratch,
...
void SoPlex::_decompResolveWithoutPreprocessing(SPxSolver& solver, SLUFactor& sluFactor,
      SPxSimplifier::Result result)
{
   ...
   if(_simplifier != 0)
   {
      ...
      if(!vanished)
      {
         assert(solver.status() == SPxSolver::OPTIMAL || solver.status() == SPxSolver::ABORT_DECOMP
                || solver.status() == SPxSolver::ABORT_EXDECOMP);
      ...
      try
      {
         _simplifier->unsimplify(primal, dual, slacks, redCost, _basisStatusRows.get_ptr(),
                                 _basisStatusCols.get_ptr(), solver.status() == SPxSolver::OPTIMAL);
...
void SoPlex::_findViolatedRows(Real compObjValue, DataArray<RowViolation>& violatedrows,
                               int& nviolatedrows)
{
   Real feastol = realParam(SoPlex::FEASTOL);
   DVector compProbRedcost(_compSolver.nCols());   // the reduced costs of the complementary problem
...
void SoPlex::_getCompatibleColumns(Vector feasVector, int* nonposind, int* compatind,
                                   int* rowsforremoval,
...
void SoPlex::_evaluateSolutionDecomp(SPxSolver& solver, SLUFactor& sluFactor,
                                     SPxSimplifier::Result result)
{
...
void SoPlex::printDecompDisplayLine(SPxSolver& solver, const SPxOut::Verbosity origVerb, bool force,
                                    bool forceHead)
{
   // setting the verbosity level
...
void SoPlex::_writeOriginalProblemBasis(const char* filename, NameSet* rowNames, NameSet* colNames,
                                        bool cpxFormat)
{
