...
template <>
void SoPlexBase<Real>::_updateDecompReducedProblem(Real objValue, DVector dualVector,
      DVector redcostVector,
      DVector compPrimalVector, DVector compDualVector);

template <>
void SoPlexBase<Real>::_decompSimplifyAndSolve(SPxSolverBase<Real>& solver, SLUFactor& sluFactor,
...
template <>
void SoPlexBase<Real>::_findViolatedRows(Real compObjValue, DataArray<RowViolation>& violatedrows,
      int& nviolatedrows);
...
template <>
void SoPlexBase<Real>::_getZeroDualMultiplierIndices(Vector feasVector, int* nonposind,
      int* colsforremoval,
      int* nnonposind, bool& stop);

template <>
void SoPlexBase<Real>::_getCompatibleColumns(Vector feasVector, int* nonposind, int* compatind,
      int* rowsforremoval,
...
template <>
void SoPlexBase<Real>::_getCompatibleBoundCons(LPRowSet& boundcons, int* compatboundcons,
      int* nonposind,
      int* ncompatboundcons, int nnonposind, bool& stop);

template <>
void SoPlexBase<Real>::_deleteAndUpdateRowsComplementaryProblem(SPxRowId rangedRowIds[],
      int& naddedrows);
...
template <>
void SoPlexBase<Real>::_evaluateSolutionDecomp(SPxSolverBase<Real>& solver, SLUFactor& sluFactor,
      typename SPxSimplifier<Real>::Result result);
...
template <>
void SoPlexBase<Real>::printDecompDisplayLine(SPxSolverBase<Real>& solver,
...
void SoPlexBase<Real>::_solveDecompositionDualSimplex()
{
   ...
   do
   {
            ...
            || _solver.status() == SPxSolverBase<Real>::ABORT_EXDECOMP || numDegenCheck > MAX_DEGENCHECK)
      {
         ...
         if(_solver.status() == SPxSolverBase<Real>::UNBOUNDED
               || _solver.status() == SPxSolverBase<Real>::INFEASIBLE)
            _hasBasis = false;
      ...
      else if(_solver.status() == SPxSolverBase<Real>::ABORT_TIME
              || _solver.status() == SPxSolverBase<Real>::ABORT_ITER
   ...
   const SPxOut::Verbosity decomp_verbosity = (SPxOut::Verbosity)intParam(
            SoPlexBase<Real>::DECOMP_VERBOSITY);
   ...
   while(!stop)
   {
      ...
      stop = decompTerminate(realParam(
                                SoPlexBase<Real>::TIMELIMIT));  // checking whether the algorithm should terminate
...
void SoPlexBase<Real>::_formDecompComplementaryProblem()
{
   ...
   if(boolParam(
            SoPlexBase<Real>::USECOMPDUAL))     // if we use the dual formulation of the complementary problem, we must
      ...
      bool fromScratch, bool applyPreprocessing)
{
   if(realParam(SoPlexBase<Real>::TIMELIMIT) < realParam(SoPlexBase<Real>::INFTY))
      solver.setTerminationTime(realParam(SoPlexBase<Real>::TIMELIMIT) -
   ...
   if(_simplifier != 0)
   {
      result = _simplifier->simplify(solver, realParam(SoPlexBase<Real>::EPSILON_ZERO),
                                     realParam(SoPlexBase<Real>::FEASTOL),
...
template <>
void SoPlexBase<Real>::_decompResolveWithoutPreprocessing(SPxSolverBase<Real>& solver,
      SLUFactor& sluFactor, typename SPxSimplifier<Real>::Result result)
{
   ...
   if(_simplifier != 0)
   {
      ...
      bool vanished = result == SPxSimplifier<Real>::VANISHED;
      ...
      if(!vanished)
      {
         assert(solver.status() == SPxSolverBase<Real>::OPTIMAL
                || solver.status() == SPxSolverBase<Real>::ABORT_DECOMP
                || solver.status() == SPxSolverBase<Real>::ABORT_EXDECOMP);
      ...
      try
      {
         _simplifier->unsimplify(primal, dual, slacks, redCost, _basisStatusRows.get_ptr(),
                                 _basisStatusCols.get_ptr(), solver.status() == SPxSolverBase<Real>::OPTIMAL);
      ...
      DVector compPrimalVector, DVector compDualVector)
{
   ...
   for(int i = 0; i < _nPrimalRows; i++)
   {
      ...
      if(_decompReducedProbRows[rownumber] && _solver.isBasic(_decompReducedProbRowIDs[rownumber]))
      {
         ...
         if(varSign == SoPlexBase<Real>::IS_FREE || (varSign == SoPlexBase<Real>::IS_POS
               && LE(compProbPrimal, 0, feastol)) ||
   ...
   for(int i = 0; i < _nPrimalRows; i++)
   {
      ...
      if(!_decompReducedProbRows[rownumber])
      {
                        ...
                        _decompPrimalRowIDs[i + 1])))
         {
            i++;
            compSlackCoeff = getCompSlackVarCoeff(i);
            Real tempViol = compProbRedcost[_compSolver.number(SPxColId(
                                               _decompDualColIDs[i]))]; // this is b - Ax
...
template <>
void SoPlexBase<Real>::_getRowsForRemovalComplementaryProblem(int* nonposind, int* bind,
      int* rowsforremoval,
   ...
   else
   {
      ...
      for(int i = 0; i < numRows(); i++)
      {
         if(_realLP->rowType(i) == LPRowBase<Real>::RANGE || _realLP->rowType(i) == LPRowBase<Real>::EQUAL)
         {
            ...
            assert(_compSolver.rowType(i) == LPRowBase<Real>::RANGE
                   || _compSolver.rowType(i) == LPRowBase<Real>::EQUAL);
      ...
      if(!_decompReducedProbRows[rowNumber])
      {
         if(_realLP->rowType(_decompPrimalRowIDs[i]) == LPRowBase<Real>::EQUAL)
         {
            ...
            LPColBase<Real> addEqualityCol(-_realLP->rhs(_decompPrimalRowIDs[i]),
                                           Real(-1.0)*_compSolver.colVector(_decompDualColIDs[i]), infinity,
...
void SoPlexBase<Real>::getOriginalProblemBasisRowStatus(DataArray< int >& degenerateRowNums,
      DataArray< typename SPxSolverBase<Real>::VarStatus >& degenerateRowStatus, int& nDegenerateRows,
      int& nNonBasicRows)
{
   Real feastol = realParam(SoPlexBase<Real>::FEASTOL);
   int basicRow = 0;
...
template <>
void SoPlexBase<Real>::_writeOriginalProblemBasis(const char* filename, NameSet* rowNames,
      NameSet* colNames, bool cpxFormat)
{
   ...
   DataArray< typename SPxSolverBase<Real>::VarStatus >
