...
namespace soplex
{
  ...
  auto parseArgs(int argc, char* argv[]) -> int
  {
    ...
    realParam.add_options()
      ("real:feastol", po::value<double>()->default_value(1e-6)->notifier(checkRange(0.0, 0.1, "real:feastol")),"primal feasibility tolerance")
      ("real:opttol", po::value<double>()->default_value(1e-6)->notifier(checkRange(0.0, 1.0, "real:opttol")), "dual feasibility tolerance")
      ("real:epsilon_zero", po::value<double>()->default_value(DEFAULT_EPS_ZERO)->notifier(checkRange(0.0, 1.0, "real:epsilon_zero")), "general zero tolerance")
      ("real:epsilon_factorization", po::value<double>()->default_value(DEFAULT_EPS_FACTOR)->notifier(checkRange(0.0, 1.0, "real:epsilon_factorization")), "zero tolerance used in factorization")
      ("real:epsilon_update", po::value<double>()->default_value(DEFAULT_EPS_UPDATE)->notifier(checkRange(0.0, 1.0, "real:epsilon_update")), "zero tolerance used in update of the factorization")
      ("real:epsilon_pivot", po::value<double>()->default_value(DEFAULT_EPS_PIVOT)->notifier(checkRange(0.0, 1.0, "real:epsilon_pivot")), "pivot zero tolerance used in factorization")
      ("real:infty", po::value<double>()->default_value(DEFAULT_INFINITY)->notifier(checkRange(1e10, 1e100, "real:infty")), "infinity threshold")
      ("real:timelimit", po::value<double>()->default_value(DEFAULT_INFINITY)->notifier(checkRange(0.0, DEFAULT_INFINITY, "real:timelimit")), "time limit in seconds")
      ("real:objlimit_lower", po::value<double>()->default_value(-DEFAULT_INFINITY)->notifier(checkRange(-DEFAULT_INFINITY, DEFAULT_INFINITY, "real:objlimit_lower")), "lower limit on objective value")
      ("real:objlimit_upper", po::value<double>()->default_value(DEFAULT_INFINITY)->notifier(checkRange(-DEFAULT_INFINITY, DEFAULT_INFINITY, "real:objlimit_upper")), "upper limit on objective value")
      ("real:fpfeastol", po::value<double>()->default_value(1e-9)->notifier(checkRange(1e-12, 1.0, "real:fpfeastol")), "working tolerance for feasibility in floating-point solver during iterative refinement")
      ("real:fpopttol", po::value<double>()->default_value(1e-9)->notifier(checkRange(1e-12, 1.0, "real:fpopttol")), "working tolerance for optimality in floating-point solver during iterative refinement")
      ("real:maxscaleincr", po::value<double>()->default_value(1e25)->notifier(checkRange(1.0, DEFAULT_INFINITY, "real:maxscaleincr")), "maximum increase of scaling factors between refinements")
      ("real:liftminval", po::value<double>()->default_value(0.000976562)->notifier(checkRange(0.0, 0.1, "real:liftminval")), "lower threshold in lifting (nonzero matrix coefficients with smaller absolute value will be reformulated)")
      ("real:liftmaxval", po::value<double>()->default_value(1024.0)->notifier(checkRange(10.0, DEFAULT_INFINITY, "real:liftmaxval")), "lower threshold in lifting (nonzero matrix coefficients with smaller absolute value will be reformulated)")
      ("real:sparsity_threshold", po::value<double>()->default_value(0.6)->notifier(checkRange(0.0, 1.0, "real:sparsity_threshold")), "sparse pricing threshold (#violations < dimension * SPARSITY_THRESHOLD activates sparse pricing)")
      ("real:representation_switch", po::value<double>()->default_value(1.2)->notifier(checkRange(0.0, DEFAULT_INFINITY, "real:representation_switch")), "threshold on number of rows vs. number of columns for switching from column to row representations in auto mode")
      ("real:ratrec_freq", po::value<double>()->default_value(1.2)->notifier(checkRange(1.0, DEFAULT_INFINITY, "real:ratrec_freq")), "geometric frequency at which to apply rational reconstruction")
      ("real:minred", po::value<double>()->default_value(1e-4)->notifier(checkRange(0.0, 1.0, "real:minred")), "minimal reduction (sum of removed rows/cols) to continue simplification")
      ("real:refac_basis_nnz", po::value<double>()->default_value(10.0)->notifier(checkRange(1.0, 100.0, "real:refac_basis_nnz")), "refactor threshold for nonzeros in last factorized basis matrix compared to updated basis matrix")
  ("real:refac_update_fill", po::value<double>()->default_value(5.0)->notifier(checkRange(1.0, 100.0, "real:refac_update_fill")), "refactor threshold for fill-in in current factor update compared to fill-in in last factorization")
  ("real:refac_mem_factor", po::value<double>()->default_value(1.5)->notifier(checkRange(1.0, 10.0, "real:refac_mem_factor")), "refactor threshold for memory growth in factorization since last refactorization")
  ("real:leastsq_acrcy", po::value<double>()->default_value(1000.0)->notifier(checkRange(1.0, DEFAULT_INFINITY, "real:leastsq_acrcy")), "accuracy of conjugate gradient method in least squares scaling (higher value leads to more iterations)")
  ("real:obj_offset", po::value<double>()->default_value(0.0)->notifier(checkRange(-DEFAULT_INFINITY, DEFAULT_INFINITY, "real:obj_offset")), "objective offset to be used");
