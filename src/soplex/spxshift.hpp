   ...
   int incr)
{
   ...
   if(fullPerturbation)
   {
      ...
      for(i = uvec.dim() - start - 1; i >= 0; i -= incr)
      {
         ...
         if(LT(u, this->inftyValue(), eps) && NE(l, u, eps) && u <= x + eps)
         {
            p_up[i] = x + random.next((double) minrandom, (double)maxrandom);
            theShift += p_up[i] - u;
         }

         if(GT(l, -this->inftyValue(), eps) && NE(l, u, eps) && l >= x - eps)
         {
   ...
   else
   {
      ...
      for(int j = uvec.delta().size() - start - 1; j >= 0; j -= incr)
      {
         ...
         if(x < -eps)
         {
            if(LT(u, this->inftyValue(), eps) && NE(l, u, eps) && vec[i] >= u - eps)
            {
         ...
         {
            if(GT(l, -this->inftyValue(), eps) && NE(l, u, eps) && vec[i] <= l + eps)
            {
   ...
   int incr)
{
   ...
   if(fullPerturbation)
   {
      ...
      for(i = uvec.dim() - start - 1; i >= 0; i -= incr)
      {
         ...
         if(LT(u, this->inftyValue(), eps) && NE(l, u, eps) && u <= x + eps && rep() * stat[i] < 0)
         {
            p_up[i] = vec[i] + random.next((double)minrandom, (double)maxrandom);
            l_theShift += p_up[i] - u;
         }

         if(GT(l, -this->inftyValue(), eps) && NE(l, u, eps) && l >= x - eps && rep() * stat[i] < 0)
         {
            p_low[i] = vec[i] - random.next((double)minrandom, (double)maxrandom);
   ...
   else
   {
      ...
      for(int j = uvec.delta().size() - start - 1; j >= 0; j -= incr)
      {
         ...
         if(x < -eps)
         {
            if(LT(u, this->inftyValue(), eps) && NE(l, u, eps) && vec[i] >= u - eps && rep() * stat[i] < 0)
            {
               p_up[i] = vec[i] + random.next((double)minrandom, (double)maxrandom);
               l_theShift += p_up[i] - u;
            }
         ...
         {
            if(GT(l, -this->inftyValue(), eps) && NE(l, u, eps) && vec[i] <= l + eps && rep() * stat[i] < 0)
            {
               p_low[i] = vec[i] - random.next((double)minrandom, (double)maxrandom);
               l_theShift -= p_low[i] - l;
            }
