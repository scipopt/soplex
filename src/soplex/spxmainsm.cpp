...
template <>
bool SPxMainSM<Real>::PostStep::checkBasisDim(DataArray<typename SPxSolverBase<Real>::VarStatus>
      rows,
      ...
      else if(EQrel(newLo, newUp, this->eps()))
      {
                 ...
                 || (!EQrel(m_oldLo, x[m_j], this->eps()) && !(EQrel(m_oldUp, x[m_j], this->eps()))))
         {
            ...
            rStatus[m_i] = (EQrel(m_lhs, x[m_j] * aij,
                                  this->eps())) ? SPxSolverBase<Real>::ON_LOWER : SPxSolverBase<Real>::ON_UPPER;
      ...
      DataArray<typename SPxSolverBase<Real>::VarStatus>& rStatus, bool isOptimal) const
{
   ...
   else
   {
      ...
      cStatus[m_j] = EQrel(m_val, m_lower) ? SPxSolverBase<Real>::ON_LOWER : (EQrel(m_val,
                     m_upper) ? SPxSolverBase<Real>::ON_UPPER : SPxSolverBase<Real>::ZERO);
   }
      ...
      DataArray<typename SPxSolverBase<Real>::VarStatus>& rStatus, bool isOptimal) const
{
   ...
   for(int k = 0; k < m_col.size(); ++k)
   {
      ...
      else
      {
         ...
         if(m_loFree)
            rStatus[m_col.index(k)] = (m_col.value(k) > 0) ? SPxSolverBase<Real>::ON_UPPER :
                                      SPxSolverBase<Real>::ON_LOWER;
         else
            rStatus[m_col.index(k)] = (m_col.value(k) > 0) ? SPxSolverBase<Real>::ON_LOWER :
                                      SPxSolverBase<Real>::ON_UPPER;
      }
   }
      ...
      DataArray<typename SPxSolverBase<Real>::VarStatus>& rStatus, bool isOptimal) const
{
   ...
   for(int k = 0; k < m_scale.size(); ++k)
   {
      ...
      if(rStatus[m_i] == SPxSolverBase<Real>::FIXED && (i == m_maxLhsIdx || i == m_minRhsIdx))
      {
         ...
         {
            rStatus[i] = m_scale.value(k) * m_scale.value(0) > 0 ? SPxSolverBase<Real>::ON_LOWER :
                         SPxSolverBase<Real>::ON_UPPER;
         }
         else
         {
            ...
            rStatus[i] = m_scale.value(k) * m_scale.value(0) > 0 ? SPxSolverBase<Real>::ON_UPPER :
                         SPxSolverBase<Real>::ON_LOWER;
         }
      ...
      DataArray<typename SPxSolverBase<Real>::VarStatus>& rStatus, bool isOptimal) const
{
   ...
   // basis:
   if(((cStatus[active_idx] == SPxSolverBase<Real>::ON_UPPER
         || cStatus[active_idx] == SPxSolverBase<Real>::FIXED)
         && NE(x[active_idx], m_oldupper, eps())) ||
         ((cStatus[active_idx] == SPxSolverBase<Real>::ON_LOWER
           || cStatus[active_idx] == SPxSolverBase<Real>::FIXED)
...
template <>
void SPxMainSM<Real>::computeMinMaxResidualActivity(SPxLP& lp, int rowNumber, int colNumber,
      Real& minAct, Real& maxAct)
{
...
template <>
void SPxMainSM<Real>::computeMinMaxValues(SPxLP& lp, Real side, Real val, Real minRes, Real maxRes,
...
template <>
typename SPxSimplifier<Real>::Result SPxMainSM<Real>::removeRowSingleton(SPxLP& lp,
      const SVector& row, int& i)
{
   assert(row.size() == 1);
   ...
   m_hist.append(new(RowSingletonPSptr) RowSingletonPS(lp, i, j, stricterLo, stricterUp, lp.lower(j),
...
template <>
typename SPxSimplifier<Real>::Result SPxMainSM<Real>::aggregateVars(SPxLP& lp, const SVector& row,
      int& i)
{
   assert(row.size() == 2);
   ...
   for(int i = lp.nRows() - 1; i >= 0; --i)
   {
      ...
      if(rhsCnt <= 1 || lhsCnt <= 1)
      {
         for(int k = 0; k < row.size(); ++k)
         {
            ...
            if(aij > 0.0)
            {
               if(lp.lhs(i) > -infinity && lp.lower(j) > -infinity && rhsCnt <= 1
               ...
               if(lp.rhs(i) < infinity && lp.upper(j) < infinity && lhsCnt <= 1
            ...
            {
               if(lp.lhs(i) > -infinity && lp.upper(j) < infinity && rhsCnt <= 1
                     && NErel(lp.lhs(i), rhsBnd, feastol())
                     // do not perform if strongly different orders of magnitude occur
                     && spxAbs(lp.lhs(i) / maxAbs(rhsBnd, 1.0)) > Param::epsilon())
               {
               ...
               if(lp.rhs(i) < infinity && lp.lower(j) > -infinity && lhsCnt <= 1
                     && NErel(lp.rhs(i), lhsBnd, feastol())
                     // do not perform if strongly different orders of magnitude occur
                     && spxAbs(lp.rhs(i) / maxAbs(lhsBnd, 1.0)) > Param::epsilon())
               {
      ...
      if(NErel(lp.lower(j), lp.upper(j), feastol()))
      {
         ...
         else if(isZero(lp.maxObj(j), epsZero()))
         {
            ...
            if(unconstrained_below || unconstrained_above)
            {
               ...
               m_hist.append(new(FreeZeroObjVariablePSptr) FreeZeroObjVariablePS(lp, j, unconstrained_below,
                             col_idx_sorted));
   ...
   for(int j = lp.nCols() - 1; j >= 0; --j)
   {
      ...
      if(upLocks[j] == 1 || downLocks[j] == 1)
      {
         ...
         if(bestpos >= 0)
         {
            ...
            Real aggConstant = (bestislhs ? lp.lhs(bestpos) : lp.rhs(
      ...
      if(m_dupRows[k].size() > 1 && !(lp.rowVector(m_dupRows[k].index(0)).size() == 1))
      {
         ...
         if(rowIdx != -1)
         {
            ...
            if(k == idxLastDupRows)
            {
               ...
               m_hist.append(new(DuplicateRowsPSptr) DuplicateRowsPS(lp, rowIdx, maxLhsIdx, minRhsIdx,
                             m_dupRows[k], scale, da_perm, isLhsEqualRhs, true, EQrel(newLhs, newRhs), k == idxFirstDupRows));
            }
            else
            {
...
template <>
typename SPxSimplifier<Real>::Result SPxMainSM<Real>::simplify(SPxLP& lp, Real eps, Real ftol,
      Real otol, bool keepbounds)
{
...
void SPxMainSM<Real>::unsimplify(const Vector& x, const Vector& y, const Vector& s, const Vector& r,
                                 const typename SPxSolverBase<Real>::VarStatus rows[],
                                 const typename SPxSolverBase<Real>::VarStatus cols[], bool isOptimal)
{
