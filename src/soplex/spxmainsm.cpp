   ...
   for(int k = 0; k < m_row.size(); ++k)
   {
      ...
      case SPxSolver::FIXED:
         if(m_fixed[k])
         {
            ...
            if(violation > maxViolation && ((EQrel(oldLo, x[cIdx], eps()) && r[cIdx] < -eps())
                                            || (EQrel(oldUp, x[cIdx], eps()) && r[cIdx] > eps())))
            {
                                       ...
                                       DataArray<SPxSolver::VarStatus>& rStatus, bool isOptimal) const
{
   ...
   else
   {
      ...
      cStatus[m_j] = EQrel(m_val, m_lower) ? SPxSolver::ON_LOWER : (EQrel(m_val,
                     m_upper) ? SPxSolver::ON_UPPER : SPxSolver::ZERO);
   }
      ...
      if(NE(lo, up))
      {
         ...
         while(i < col.size())
         {
            ...
            else
            {
               if(aij > maxVal)
               {
                  MSG_WARNING((*spxout), (*spxout) << "WMAISM05 Warning! Big matrix coefficient " << aij <<
                              std::endl);
               }
               else if(isZero(aij, tol))
               {
                  MSG_WARNING((*spxout), (*spxout) << "WMAISM06 Warning! Tiny matrix coefficient " << aij <<
                              std::endl);
               }
...
void SPxMainSM::computeMinMaxResidualActivity(SPxLP& lp, int rowNumber, int colNumber, Real& minAct,
      Real& maxAct)
{
...
void SPxMainSM::computeMinMaxValues(SPxLP& lp, Real side, Real val, Real minRes, Real maxRes,
...
SPxSimplifier::Result SPxMainSM::simplify(SPxLP& lp, Real eps, Real ftol, Real otol,
      bool keepbounds)
{
