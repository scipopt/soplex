...
void SoPlex::_optimizeRational()
{
   ...
   if(realParam(SoPlex::OBJLIMIT_LOWER) > -realParam(SoPlex::INFTY)
   ...
   if(_status == SPxSolver::OPTIMAL || _status == SPxSolver::INFEASIBLE
         || _status == SPxSolver::UNBOUNDED)
   ...
   bool& error)
{
   ...
   do
   {
      ...
      for(int c = numColsRational() - 1; c >= 0; c--)
      {
         ...
         if(((maximizing && basisStatusCol != SPxSolver::ON_LOWER) || (!maximizing
               && basisStatusCol != SPxSolver::ON_UPPER))
         ...
         if(((maximizing && basisStatusCol != SPxSolver::ON_UPPER) || (!maximizing
               && basisStatusCol != SPxSolver::ON_LOWER))
               && sol._redCost[c] > redCostViolation)
         {
            MSG_DEBUG(std::cout << "basisStatusCol = " << basisStatusCol
      ...
      for(int r = numRowsRational() - 1; r >= 0; r--)
      {
         ...
         if(((maximizing && basisStatusRow != SPxSolver::ON_LOWER) || (!maximizing
         ...
         if(((maximizing && basisStatusRow != SPxSolver::ON_UPPER) || (!maximizing
               && basisStatusRow != SPxSolver::ON_LOWER))
               && sol._dual[r] > dualViolation)
         {
            MSG_DEBUG(std::cout << "basisStatusRow = " << basisStatusRow
      ...
      MSG_DEBUG(spxout
                ...
                << std::setw(10) << rationalToString(boundsViolation > sideViolation ? boundsViolation :
                      sideViolation, 2) << " & "
                << std::setw(10) << rationalToString(redCostViolation > dualViolation ? redCostViolation :
                      dualViolation, 2) << "\n");
...
#ifndef NDEBUG
      {
         ...
         if(debugRedCostViolation > _solver.opttol() || debugDualViolation > _solver.opttol()
...
void SoPlex::_lift()
{
   ...
   for(int i = 0; i < numColsRational(); i++)
   {
      ...
      for(int k = colVector.size() - 1; k >= 0; k--)
      {
         ...
         {
            MSG_DEBUG(std::cout << "   --> nonzero " << k << " has value " << rationalToString(
                         value) << " in row " << colVector.index(k) << "\n");
...
void SoPlex::_untransformUnbounded(SolRational& sol, bool unbounded)
{
   ...
   if(unbounded)
   {
      ...
      _hasBasis = (_basisStatusCols[numOrigCols] != SPxSolver::BASIC
                   && _basisStatusRows[numOrigRows] == SPxSolver::BASIC);
      _basisStatusCols.reSize(numOrigCols);
...
void SoPlex::_transformFeasibility()
{
   ...
   _rationalLP->addCol(id,
                       LPColRational((intParam(SoPlex::OBJSENSE) == SoPlex::OBJSENSE_MAXIMIZE ? _rationalPosone :
                                      _rationalNegone),
                                     _tauColVector, 1, 0));
   _realLP->addCol(id,
...
void SoPlex::_computeInfeasBox(SolRational& sol, bool transformed)
{
   ...
   MSG_DEBUG(std::cout << "max ytransA*[x_l,x_u] = " << (isTempFinite ? rationalToString(
                temp) : "infinite") << "\n");
...
SPxSolver::Status SoPlex::_solveRealForRational(bool fromscratch, VectorReal& primal,
      VectorReal& dual, DataArray< SPxSolver::VarStatus >& basisStatusRows,
   ...
   try
   {
      ...
      if(_simplifier != 0)
      {
         ...
         simplificationStatus = _simplifier->simplify(_solver, realParam(SoPlex::EPSILON_ZERO),
                                realParam(SoPlex::FPFEASTOL), realParam(SoPlex::FPOPTTOL), keepbounds);
      }
      ...
      if(simplificationStatus == SPxSimplifier::INFEASIBLE)
         ...
         result = SPxSolver::UNBOUNDED;
              ...
              || simplificationStatus == SPxSimplifier::OKAY)
      {
...
SPxSolver::Status SoPlex::_solveRealStable(bool acceptUnbounded, bool acceptInfeasible,
      ...
      const bool forceNoSimplifier)
{
   ...
   while(true)
   {
      ...
      if(!turnedoffPre
            && (intParam(SoPlex::SIMPLIFIER) != SoPlex::SIMPLIFIER_OFF
                || intParam(SoPlex::SCALER) != SoPlex::SCALER_OFF))
      {
...
void SoPlex::_factorizeColumnRational(SolRational& sol,
                                      ...
                                      bool& error, bool& optimal)
{
   ...
   for(int i = 0; i < basisStatusRows.size(); i++)
   {
      ...
      {
         if(((maximizing && basisStatusRows[i] != SPxSolver::ON_LOWER) || (!maximizing
               && basisStatusRows[i] != SPxSolver::ON_UPPER))
               && (basisStatusRows[i] != SPxSolver::ZERO || rhsRational(i) != 0))
         {
            ...
            MSG_DEBUG(spxout << "negative dual multliplier for row " << i
                      ...
                      << " and [lhs,rhs] = [" << rationalToString(lhsRational(i)) << "," << rationalToString(rhsRational(
                               i)) << "]"
                      << "\n");
         }
      }
      ...
      {
         if(((maximizing && basisStatusRows[i] != SPxSolver::ON_UPPER) || (!maximizing
               && basisStatusRows[i] != SPxSolver::ON_LOWER))
   ...
   for(int i = 0; i < basisStatusCols.size(); i++)
   {
      ...
      else
      {
         ...
         {
            if(((maximizing && basisStatusCols[i] != SPxSolver::ON_LOWER) || (!maximizing
                  && basisStatusCols[i] != SPxSolver::ON_UPPER))
         ...
         else if(_workSol._redCost[i] < 0)
         {
            ...
            if(((maximizing && basisStatusCols[i] != SPxSolver::ON_UPPER) || (!maximizing
                  && basisStatusCols[i] != SPxSolver::ON_LOWER))
...
bool SoPlex::_reconstructSolutionRational(SolRational& sol,
      DataArray< SPxSolver::VarStatus >& basisStatusRows,
      DataArray< SPxSolver::VarStatus >& basisStatusCols, const Rational& denomBoundSquared)
{
   ...
   for(int c = numColsRational() - 1; c >= 0; c--)
   {
      ...
      {
         MSG_DEBUG(std::cout << "Lower bound of variable " << c << " violated by " << rationalToString(
                      lowerRational(c) - _workSol._primal[c]) << "\n");
      ...
      {
         MSG_DEBUG(std::cout << "Upper bound of variable " << c << " violated by " << rationalToString(
   ...
   for(int r = numRowsRational() - 1; r >= 0; r--)
   {
      ...
      {
         MSG_DEBUG(std::cout << "Lhs of row " << r << " violated by " << rationalToString(lhsRational(
                      r) - _workSol._slacks[r]) << "\n");
      ...
      {
         MSG_DEBUG(std::cout << "Rhs of row " << r << " violated by " << rationalToString(
