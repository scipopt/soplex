...
)
{
   ...
   if(!enableBoundFlips || polish || this->thesolver->rep() == SPxSolverBase<Real>::ROW
         || flipPotential <= 0)
   {
...
)
{
   ...
   if(polish || !enableBoundFlips || !enableRowBoundFlips
         || this->thesolver->rep() == SPxSolverBase<Real>::COLUMN || flipPotential <= 0)
   {
   ...
   const Real*  vec =
      this->thesolver->fVec().get_const_ptr();         /**< pointer to values of current vector */
   const Real*  upd =
      this->thesolver->fVec().delta().values();        /**< pointer to update values of current vector */
   const int*   idx =
      this->thesolver->fVec().delta().indexMem();      /**< pointer to indices of current vector */
   int          updnnz =
      this->thesolver->fVec().delta().size();       /**< number of nonzeros in update vector */
   const Real*  lb  =
      this->thesolver->lbBound().get_const_ptr();      /**< pointer to lower bound/lhs of current vector */
   const Real*  ub  =
      this->thesolver->ubBound().get_const_ptr();      /**< pointer to upper bound/rhs of current vector */
