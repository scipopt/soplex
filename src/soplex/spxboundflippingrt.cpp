...
)
{
   ...
   if(polish || !enableBoundFlips || !enableRowBoundFlips || thesolver->rep() == SPxSolver::COLUMN
         || flipPotential <= 0)
   {
   ...
   const Real*  vec =
      thesolver->fVec().get_const_ptr();         /**< pointer to values of current vector */
   const Real*  upd =
      thesolver->fVec().delta().values();        /**< pointer to update values of current vector */
   const int*   idx =
      thesolver->fVec().delta().indexMem();      /**< pointer to indices of current vector */
   int          updnnz =
      thesolver->fVec().delta().size();       /**< number of nonzeros in update vector */
   const Real*  lb  =
      thesolver->lbBound().get_const_ptr();      /**< pointer to lower bound/lhs of current vector */
   const Real*  ub  =
      thesolver->ubBound().get_const_ptr();      /**< pointer to upper bound/rhs of current vector */
