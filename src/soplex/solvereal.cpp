...
template <>
void SoPlexBase<Real>::_evaluateSolutionReal(typename SPxSimplifier<Real>::Result
...
void SoPlexBase<Real>::_optimize()
{
   ...
   if(!_hasBasis && realParam(SoPlexBase<Real>::OBJLIMIT_LOWER) == -realParam(SoPlexBase<Real>::INFTY)
...
void SoPlexBase<Real>::_storeSolutionReal(bool verify)
{
   ...
   assert(_solver.basis().status() != SPxBasisBase<Real>::PRIMAL
          || status() != SPxSolverBase<Real>::ERROR);
   assert(_solver.basis().status() != SPxBasisBase<Real>::PRIMAL
          || status() != SPxSolverBase<Real>::NO_RATIOTESTER);
   assert(_solver.basis().status() != SPxBasisBase<Real>::PRIMAL
          || status() != SPxSolverBase<Real>::NO_PRICER);
   assert(_solver.basis().status() != SPxBasisBase<Real>::PRIMAL
          || status() != SPxSolverBase<Real>::NO_SOLVER);
   assert(_solver.basis().status() != SPxBasisBase<Real>::PRIMAL
          || status() != SPxSolverBase<Real>::NOT_INIT);
   assert(_solver.basis().status() != SPxBasisBase<Real>::PRIMAL
          || status() != SPxSolverBase<Real>::SINGULAR);
   assert(_solver.basis().status() != SPxBasisBase<Real>::PRIMAL
          || status() != SPxSolverBase<Real>::NO_PROBLEM);
   assert(_solver.basis().status() != SPxBasisBase<Real>::PRIMAL
          || status() != SPxSolverBase<Real>::UNBOUNDED);
   assert(_solver.basis().status() != SPxBasisBase<Real>::PRIMAL
          || status() != SPxSolverBase<Real>::INFEASIBLE);
   assert(_solver.basis().status() != SPxBasisBase<Real>::UNBOUNDED
          || status() == SPxSolverBase<Real>::UNBOUNDED);
   assert(_solver.basis().status() == SPxBasisBase<Real>::UNBOUNDED
          || _solver.basis().status() == SPxBasisBase<Real>::NO_PROBLEM
          || status() != SPxSolverBase<Real>::UNBOUNDED);

   _solReal._isPrimalFeasible = (status() == SPxSolverBase<Real>::OPTIMAL
                                 || ((_solver.basis().status() == SPxBasisBase<Real>::PRIMAL
                                      || _solver.basis().status() == SPxBasisBase<Real>::UNBOUNDED)
   ...
   assert(_solver.basis().status() != SPxBasisBase<Real>::DUAL
          || status() != SPxSolverBase<Real>::ERROR);
   assert(_solver.basis().status() != SPxBasisBase<Real>::DUAL
          || status() != SPxSolverBase<Real>::NO_RATIOTESTER);
   assert(_solver.basis().status() != SPxBasisBase<Real>::DUAL
   ...
   assert(_solver.basis().status() != SPxBasisBase<Real>::DUAL
          || status() != SPxSolverBase<Real>::INFEASIBLE);
   assert(_solver.basis().status() != SPxBasisBase<Real>::INFEASIBLE
          || status() == SPxSolverBase<Real>::INFEASIBLE);
   assert(_solver.basis().status() == SPxBasisBase<Real>::INFEASIBLE
          || _solver.basis().status() == SPxBasisBase<Real>::NO_PROBLEM
          || status() != SPxSolverBase<Real>::INFEASIBLE);
   ...
   _solReal._isDualFeasible = (status() == SPxSolverBase<Real>::OPTIMAL
                               || ((_solver.basis().status() == SPxBasisBase<Real>::DUAL
                                    || _solver.basis().status() == SPxBasisBase<Real>::INFEASIBLE)
                                   && _solver.shift() < 10.0 * realParam(SoPlexBase<Real>::EPSILON_ZERO)));
