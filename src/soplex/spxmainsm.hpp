...
bool SPxMainSM<R>::checkSolution(SPxLPBase<R>& lp, DVectorBase<R> sol)
{
  ...
  template <class R>
  typename SPxSimplifier<R>::Result SPxMainSM<R>::removeRowSingleton(SPxLPBase<R>& lp, const SVectorBase<R>& row, int& i)
  {
    assert(row.size() == 1);
...
template <class R>
typename SPxSimplifier<R>::Result SPxMainSM<R>::aggregateVars(SPxLPBase<R>& lp, const SVectorBase<R>& row, int& i)
{
   assert(row.size() == 2);
